...
describe("selectionIndexAfterBackgroundOpen", () => {
  ...
  it("finds row by path after reload", () => {
    expect(
      selectionIndexAfterBackgroundOpen(repos, "", "/tmp/beta"),
    ).toBe(1);
  });

  it("respects active filter", () => {
    expect(
      selectionIndexAfterBackgroundOpen(repos, "gam", "/tmp/gamma"),
    ).toBe(0);
  });

  it("falls back to first row when path missing", () => {
    expect(
      selectionIndexAfterBackgroundOpen(repos, "", "/tmp/missing"),
    ).toBe(0);
  });
});
